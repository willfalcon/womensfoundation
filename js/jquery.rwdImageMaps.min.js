...
jQuery(document).ready(function() {
    jQuery(window).scroll(function() {
        if(jQuery(window).scrollTop() >= 380) {
            jQuery('body').addClass('sticky_lc');
        }
        else {
            jQuery('body').removeClass('sticky_lc');
        }
    });
});
...
jQuery(document).ready(function() {
	...
	jQuery('.wf_map_button').on({
  		click:function(e) {
  			e.preventDefault();
  			jQuery('.wf_cycle_img').removeClass('wf_opened_button');
  			var wf_pos=jQuery(this).attr('data-pos');
  			//console.log(wf_pos);
  			jQuery('.wf_img_'+wf_pos).addClass('wf_opened_button');
  			jQuery('.wftab-'+wf_pos+' a').trigger('click');
  		}
	});

  jQuery('.member_colored_block').on({
    click:function() {
      jQuery('.member_colored_block').removeClass('wf_bordered');
        console.log(jQuery(this).parent().attr('data-pos'));
        var wf_pos=jQuery(this).parent().attr('data-pos');
        if (jQuery(this).parent().parent().find('#wf_member_desc-'+wf_pos).hasClass('wf_opened')) {
          //CLOSE PANEL
            jQuery(this).parent().parent().find('.wf_opened').slideToggle("slow");
            jQuery(this).parent().parent().find('.wf_opened').removeClass('wf_opened');
        }
        else {
          //OPEN PANEL
            jQuery(this).parent().parent().find('.wf_opened').slideToggle("slow");
            jQuery(this).parent().parent().find('.wf_opened').removeClass('wf_opened');
            jQuery(this).parent().parent().find('#wf_member_desc-'+wf_pos).toggleClass('wf_opened');
            jQuery(this).parent().parent().find('#wf_member_desc-'+wf_pos).slideToggle( "slow");
            jQuery(this).parent().parent().find('#wf_mobile_desc-'+wf_pos).toggleClass('wf_opened');
            jQuery(this).parent().parent().find('#wf_mobile_desc-'+wf_pos).slideToggle( "slow");
            jQuery(this).addClass('wf_bordered');
        }
  }});
  jQuery('#wf_mobile_loupe').on({
    click:function(e) {
      jQuery('#prk_menu_loupe').trigger('click');
    }
  });
  jQuery('#wf_mobile_close').on({
    click:function(e) {
      jQuery('#body_hider').trigger('click');
    }
  });
  jQuery('.wf_toggler>a').on({
    click:function(e) {
      e.preventDefault();
      var fragment=jQuery(this).attr('href').split('#');
      jQuery('.'+fragment[1]+'').slideToggle({
          'duration':1000,
          easing:'easeInOutExpo'
      });
    }
  });
  jQuery('.timeline-year').on({
    click:function(e) {
      e.preventDefault();
      jQuery(this).next().find('.timeline-content .full-width').slideToggle({
          'duration':500,
          easing:'easeInOutExpo'
      });
      jQuery(this).next().find('.timeline-content .content-details').slideToggle({
          'duration':500,
          easing:'easeInOutExpo'
      });
    }
  });
  jQuery(".wf_custom_select").each(function() {
        var $thisa=jQuery(this);
        $thisa.selectOrDie({
            onChange: function() {
                //console.log($thisa.parent().find('.sod_list_wrapper span.active').attr('data-value'));
                window.location.href=$thisa.parent().find('.sod_list_wrapper span.active').attr('data-value');
            }
        });
    });
